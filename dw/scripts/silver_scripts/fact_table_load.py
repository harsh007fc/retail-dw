...
def load_fact_sales(spark, processing_date):
    ...
    dim_product = DeltaTable.forPath(spark, "../dw/destination_bucket/silver/dim_product/").toDF().filter("is_active = true")
    dim_customer = DeltaTable.forPath(spark, "../dw/destination_bucket/silver/dim_customer/").toDF().filter("is_active = true")
    dim_date = DeltaTable.forPath(spark, "../dw/destination_bucket/silver/dim_date/").toDF()
    
    # Join fact data with dimensions
    fact_sales = orders_df.join(order_items_df, "order_id") \
        .join(dim_product, 
...
def main():
    ...
    order_item_dates = get_available_dates(spark, "../dw/destination_bucket/bronze/order_items")
    ...
    if DeltaTable.isDeltaTable(spark, "fact_sales/"):
        ...
        delta_table = DeltaTable.forPath(spark, "../dw/destination_bucket/silver/fact_sales/")
