...
def load_product_dimension(spark, processing_date):
    ...
    latest_products = product_df.withColumn("row_num", row_number().over(windowSpec))\
...
def load_customer_dimension(spark, processing_date):
    ...
    latest_customers = customer_df.withColumn("row_num", row_number().over(windowSpec))\
                                 .filter(col("row_num") == 1)
    
    # Calculate loyalty tier
    order_counts = orders_df.groupBy("customer_id").agg(count("*").alias("order_count"))
    
    dim_customer = latest_customers.join(order_counts, "customer_id", "left") \
...
def process_date(spark, processing_date, bookmark_path, initial_load=False):
    ...
    if initial_load:
        ...
        dim_product.write.format("delta").mode("overwrite").save("../dw/destination_bucket/silver/dim_product/")
        dim_customer.write.format("delta").mode("overwrite").save("../dw/destination_bucket/silver/dim_customer/")
    else:
...
def main():
    ...
    customer_dates = get_available_dates(spark, "../dw/destination_bucket/bronze/customers")
