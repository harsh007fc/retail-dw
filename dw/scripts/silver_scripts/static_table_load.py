...
def generate_date_dimension(spark, start_date, end_date, processing_date):
    ...
    date_range = spark.range(
        start=start_date.toordinal(), end=end_date.toordinal()
    ).select(col("id").cast("integer").alias("day_ordinal"))

    dim_date = (
        date_range.withColumn(
            "full_date",
            to_date(col("day_ordinal") - lit(start_date.toordinal()), "yyyy-MM-dd"),
        )
        .withColumn(
            "date_sk", date_format(col("full_date"), "yyyyMMdd").cast("integer")
        )
...
def load_category_dimension(spark, processing_date):
    ...
    category_df = spark.read.parquet(
        f"dw/destination_bucket/bronze/categories/ingestion_date={date_str}/"
    )
    ...
    latest_categories = category_df.withColumn(
        "row_num", row_number().over(windowSpec)
    ).filter(col("row_num") == 1)

    dim_category = (
        latest_categories.withColumn("category_sk", monotonically_increasing_id())
...
def process_date(spark, processing_date, bookmark_path, initial_load=False):
    ...
    if dim_date:
        dim_date.write.format("delta").mode("append").save(
            "dw/destination_bucket/silver/dim_date/"
        )

    # Category dimension (SCD2)
    dim_category = load_category_dimension(spark, processing_date)
    if initial_load:
        dim_category.write.format("delta").mode("overwrite").save(
            "dw/destination_bucket/silver/dim_category/"
        )
    else:
...
def main():
    ...
    category_dates = get_available_dates(
        spark, "dw/destination_bucket/bronze/categories"
    )
